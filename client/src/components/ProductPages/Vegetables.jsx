...
const Vegetables = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const productsWithCorrectPaths = response.data.map(product => ({
        ...product,
        image: new URL(product.image, import.meta.url).href // This will resolve the image path correctly
      }));
      ...
      {products.map((product) => (
        <div className="w-full" key={product._id}>
          <Card
            ...
            name={product.name}
            image={product.image}
            quantity={product.quantity || '1 kg'} // Default quantity if not specified
            price={product.price}
